...
export function buildGoogleAdsScriptTemplate(baseUrl: string) {
  ...
  return [
    ...
    "    buildApiUrl('/api/google-ads/pending', { customerId: customerId }),",
    "    {",
    "      method: 'get',",
    ...
    "  }",
    "",
    "  var response = UrlFetchApp.fetch(buildApiUrl('/api/google-ads/report'), {",
    "    method: 'post',",
    ...
    "",
    "  if (SCRIPT_TOKEN && SCRIPT_TOKEN !== 'REPLACE_ME') {",
    "    query.push('scriptToken=' + encodeURIComponent(SCRIPT_TOKEN));",
    "  }",
    "",
