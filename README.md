...
1. 先用浏览器打开联盟链接，记录整条跳转链
2. 从跳转链里挑出最像最终落地页、而且包含参数的那个 URL
3. 从这个 URL 里取出 `?` 后面的参数
4. 生成 Google Ads 后缀
5. 后台接口把“待同步”的后缀提供给 Google Ads Script
6. Google Ads Script 把这个后缀写到对应广告系列的 `Final URL suffix`
7. 写成功后再回传后台，状态从“待同步”变成“已同步”
